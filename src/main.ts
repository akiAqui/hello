...
class App {
  private readonly videoStream: MediaStream | null = null;
  private readonly audioStream: MediaStream | null = null;
  private readonly combinedStream: MediaStream | null = null;
  ...
  // State management  private disposed = false;
  ...
  constructor() {
    ...
    this.debugMode = true; //process.env.NODE_ENV === 'development';
  ...
  private initializeAudio(): void {
    try {
      ...
      this.audioStream = audioDestination.stream; // 音声ストリームを保存
      
      // デバッグログでストリームを確認
      if (this.audioStream) {
        console.log('Audio stream initialized:', this.audioStream);
      } else {
        throw new Error('Audio stream could not be initialized');
      }
    } catch (error) {
      console.error('Failed to initialize audio:', error);
      throw error; // エラーをスローして呼び出し元に通知
    }
  }
  ...
  public startRecording(): void {
    ...
    if (!this.videoStream) {
      console.error('Video stream is not initialized');
      return;
    }
    // 動画と音声を結合
    ...
    console.log('Audio stream:', this.audioStream);
    console.log('Combined stream:', this.combinedStream);
    ...
    mediaRecorder.onstop = () => {
      ...
      a.href = url;
      a.download = 'threejs_with_audio.webm';
      a.click();
    };
    mediaRecorder.start();
    setTimeout(() => mediaRecorder.stop(), 10000); // 10秒録画
  }
  ...
  private createCamera(): THREE.PerspectiveCamera {
    ...
    //camera.lookAt(0, 0, 0);
    // 回転を設定
    const euler = new THREE.Euler(0, 0, 0, 'XYZ'); // オイラー角 (0, 0, 0) を指定
    camera.quaternion.setFromEuler(euler);
    this.cameraDirection = camera.getWorldDirection(new THREE.Vector3());
    return camera;
  }
  ...
  private createRenderer(): THREE.WebGLRenderer {
    ...
    // PCF: Percentage Closer Filtering, 影のエッジをぼかす
    // VSM: Variance Shadow Map    、分散を使用して、どのように？統計的により柔らかな影にする
    ...
    // for recording
    this.videoStream = renderer.domElement.captureStream(30); // 30fps
    this.stats = new Stats();
    document.body.appendChild(this.stats.dom);
    return renderer;
  }
...
private handleKeyDown(event: KeyboardEvent): void {
  try {
    ...
    // カメラの向きベクトルを取得（前方向）
    const forward = this.camera.getWorldDirection(new THREE.Vector3()); // 前方ベクトル
    forward.y=0;
    forward.normalize();
    console.log("forward=",forward);
    const worldUp = new THREE.Vector3(0,1,0);

    const right = new THREE.Vector3().crossVectors(forward, worldUp).normalize(); // 右方向ベクトル
    const left  = new THREE.Vector3().crossVectors(worldUp, forward).normalize(); // 左方向ベクトル
    const newright = new THREE.Vector3(forward.z,0,-forward.x);
    ...
    switch (event.key) {
      ...
      case 'c': // 後退（カメラの向いている方向の逆）
        ...
        break;
      case 's': // 左移動（右方向の逆）
        moveDirection.copy(left);
        //localMoveDirection.set(-1,0,0);//copy(new THREE.Vector3(-1,0,0));
        //localMoveDirection.applyQuaternion(cameraQuaternion); // カメラ回転適用
        break;
      case 'd': // 右移動（右方向）
    ...
    // クォータニオンを適用してワールド座標系での移動ベクトルを計算
    //let worldMoveDirection = localMoveDirection.applyQuaternion(cameraQuaternion);
    // worldMoveDirection.normalize().multiplyScalar(moveSpeed);
    // カメラ位置の更新
    ...
    newPosition = this.camera.position.clone().add(moveDirection);
    console.log("tmp new Position",newPosition);
    newPosition.y = this.camera.position.y;
    console.log("lst new Position",newPosition);    
    this.camera.position.copy(newPosition);
    //this.camera.position.add(moveDirection);
    
    console.log("moveDirection=",moveDirection);
    console.log("cam pos",this.camera.position);
    //this.camera.position.add(new THREE.Vector3(0.0001,0,0).normalize());
    //console.log("cam pos",this.camera.position);
